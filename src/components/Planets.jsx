...
const Planets = () => {
  ...
  return (
    ...
    <h2 className="flex text-3xl font-semibold mb-6 text-white text-left">Planets List</h2>
    ...
    {isGridView ? (
      ...
      <div className="grid grid-cols-3 gap-4"style={{ cursor: 'pointer' }}>
        {planetsData.map((planet,index) => (
          <div key={planet.url} className="bg-blue-100 p-4 border">
          <img src={randomImages[index % randomImages.length].download_url} alt={planet.name} className="w-full h-32 object-cover mb-2" />
      ...
      <table className="table-auto w-full">
        <thead>
          <tr className="bg-gray-600 text-white">
            ...
            <th className="px-4 py-2 font-bold">Climate</th>
            <th className="px-4 py-2 font-bold">Terrain</th>
            <th className="px-4 py-2 font-bold">Population</th>
            {/* Add more table headers here as needed */}
          </tr>
        </thead>
        <tbody>
          {planetsData.map((planet) => (
            <tr key={planet.url} className="bg-blue-100"style={{ cursor: 'pointer' }}>
